...
export class PerformanceMonitor {
  ...
  measureRender<T>(toolType: string, renderFn: () => T): T {
    ...
    if (renderTime > this.thresholds.maxRenderTime) {
      console.warn(`🐌 Slow render: ${toolType} took ${renderTime.toFixed(2)}ms (target: ${this.thresholds.maxRenderTime}ms)`);
    }
  ...
  measureInteraction<T>(interactionType: string, interactionFn: () => T): T {
    ...
    if (interactionTime > this.thresholds.maxInteractionTime) {
      console.warn(`🐌 Slow interaction: ${interactionType} took ${interactionTime.toFixed(2)}ms (target: ${this.thresholds.maxInteractionTime}ms)`);
    }
