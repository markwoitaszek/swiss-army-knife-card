...
export default class SparklineGraphTool extends BaseTool {
  constructor(argToolset, argConfig, argPos) {
    ...
    if (typeof this.config.position.margin === 'object') {
      this.svg.margin.t =
        Utils.calculateSvgDimension(this.config.position.margin?.t) ||
        Utils.calculateSvgDimension(this.config.position.margin?.y) ||
        0;
      this.svg.margin.b =
        Utils.calculateSvgDimension(this.config.position.margin?.b) ||
        Utils.calculateSvgDimension(this.config.position.margin?.y) ||
        0;
      this.svg.margin.r =
        Utils.calculateSvgDimension(this.config.position.margin?.r) ||
        Utils.calculateSvgDimension(this.config.position.margin?.x) ||
        0;
      this.svg.margin.l =
        Utils.calculateSvgDimension(this.config.position.margin?.l) ||
        Utils.calculateSvgDimension(this.config.position.margin?.x) ||
        0;
    ...
    if (this.config.sparkline?.radial_barcode?.face) {
      if (this.config.sparkline.radial_barcode.face?.show_day_night === true)
        this.svg.clockface.dayNightRadius = Utils.calculateSvgDimension(
          this.config.sparkline.radial_barcode.face.day_night_radius
        );
      if (this.config.sparkline.radial_barcode.face?.show_hour_marks === true)
        this.svg.clockface.hourMarksRadius = Utils.calculateSvgDimension(
          this.config.sparkline.radial_barcode.face.hour_marks_radius
        );
      if (
        ['absolute', 'relative'].includes(
          this.config.sparkline.radial_barcode.face?.show_hour_numbers
        )
      )
        this.svg.clockface.hourNumbersRadius = Utils.calculateSvgDimension(
          this.config.sparkline.radial_barcode.face.hour_numbers_radius
        );
    }
    this._data = [];
    this._bars = [];
    ...
    this.svg.line_width = Utils.calculateSvgDimension(
      this.config.sparkline[this.config.sparkline.show.chart_type]?.line_width ||
        this.config.line_width ||
        0
    );
    this.svg.column_spacing = Utils.calculateSvgDimension(
      this.config.sparkline[this.config.sparkline.show.chart_type]?.column_spacing ||
        this.config.bar_spacing ||
        1
    );
    this.svg.row_spacing = Utils.calculateSvgDimension(
      this.config.sparkline[this.config.sparkline.show.chart_type]?.row_spacing ||
        this.config.bar_spacing ||
        1
    );
    ...
    this.config.sparkline.colorstops.colors.map((value, index) => {
      let rankIndex;
      rankIndex =
        this.config.sparkline.show?.chart_variant === 'rank_order' && value.rank !== undefined
          ? value.rank
          : index;
    ...
    this.radialBarcodeChartWidth = Utils.calculateSvgDimension(
      this.config?.radial_barcode?.size || 5
    );
    // Graph settings
    this.svg.graph = {};
    ...
    this.config.sparkline.state_map.map.forEach((state, i) => {
      ...
      if (typeof state === 'string')
        this.config.sparkline.state_map.map[i] = { value: state, label: state };
      // make sure label is set
      this.config.sparkline.state_map.map[i].label =
        this.config.sparkline.state_map.map[i].label ||
        this.config.sparkline.state_map.map[i].value;
    });
  ...
  set series(states) {
    ...
    if (config.sparkline.show.chart_type) {
      ...
      const bound =
        this._card.config.entities[i].states === 'secondary' ? this.boundSecondary : this.bound;
      ...
      if (
        config.sparkline.show.chart_type === 'dots' ||
        ...
        this.Graph[i].valuesPerBucket =
          (this.Graph[i].max - this.Graph[i].min) / this.config.sparkline.equalizer.value_buckets;
      ...
      history[0].forEach((item, index) => {
        ...
        for (let i = 0; i < this.gradeRanks.length; i++) {
          ...
          for (let j = 0; j < this.gradeRanks[i].rangeMin.length; j++) {
            if (
              item.state >= this.gradeRanks[i].rangeMin[j] &&
              item.state < this.gradeRanks[i].rangeMax[j]
            ) {
              match = true;
              matchBucket = j;
              matchStep = i;
  ...
  get primaryYaxisEntities() {
    return this.visibleEntities.filter(
      entity => entity.states === undefined || entity.states === 'primary'
    );
  }
  ...
  intColor(inState, i) {
    ...
    if (colorstops.colors.length > 0) {
      ...
      if (this.config.sparkline.show.chart_type === 'bar') {
        const { color } =
          colorstops.colors.find(ele => ele.value < state) || colorstops.colors.slice(-1)[0];
        intColor = color;
      } else {
  ...
  setTooltip(entity, index, value, label = null) {
    ...
    const offset =
      hours_to_show < 1 && bins_per_hour < 1 ? bins_per_hour * hours_to_show : 1 / bins_per_hour;
  ...
  renderSvgTrafficLight(trafficLight, i) {
    ...
    if (this.config.sparkline.graded.square === true) {
      ...
      if (size < trafficLight.height) {
        let spaceBetween =
          (this.svg.graph.height - this.gradeRanks.length * size) / (this.gradeRanks.length - 1);
  ...
  renderSvgEqualizerMask(equalizer, index) {
    ...
    if (this.config.sparkline.equalizer.square === true) {
      ...
      if (size < equalizer[0].height) {
        let spaceBetween =
          (this.svg.height - this.config.sparkline.equalizer.value_buckets * size) /
  ...
  renderSvgRadialBarcodeBackground(radius) {
    const { start, end, start2, end2, largeArcFlag, sweepFlag } =
      this.Graph[0]._calcRadialBarcodeCoords(
        0,
        359.9,
        true,
        radius,
    ...
    const paths = barcode.map((barcodePart, i) => {
      ...
      if (this.config.sparkline.state_values?.use_value === 'bin') {
        ...
        if (this.gradeRanks[flooredValue]?.value) {
          const colorValue =
            this.gradeRanks[flooredValue].value[0] +
            (this.gradeRanks[flooredValue].rangeMax[0] -
              this.gradeRanks[flooredValue].rangeMin[0]) *
              (barcodePart.value - flooredValue);
      ...
      const realHeight =
        barcodePart.height - this.svg.margin.t - this.svg.margin.b - this.svg.line_width;
