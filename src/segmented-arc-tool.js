...
export default class SegmentedArcTool extends BaseTool {
  constructor(argToolset, argConfig, argPos) {
    ...
    this.svg.radiusX = Utils.calculateSvgDimension(
      argConfig.position.radius_x || argConfig.position.radius
    );
    this.svg.radiusY = Utils.calculateSvgDimension(
      argConfig.position.radius_y || argConfig.position.radius
    );
    ...
    if (this.config.show.style === 'fixedcolor') {
      ...
      this._segments.gap =
        this.config.segments.colorlist.gap !== 'undefined' ? this.config.segments.colorlist.gap : 1;
      ...
      for (var i = 0; i < this._segments.count; i++) {
        this._segmentAngles[i] = {
          ...
          boundsEnd:
            this.config.position.start_angle +
            (segmentRunningSize + this._segments.sizeList[i]) * this._arc.direction,
          drawStart:
            this.config.position.start_angle +
            segmentRunningSize * this._arc.direction +
            this._segments.gap * this._arc.direction,
          drawEnd:
            this.config.position.start_angle +
            (segmentRunningSize + this._segments.sizeList[i]) * this._arc.direction -
            this._segments.gap * this._arc.direction,
      ...
      if (typeof this._segments.colorStops[this.config.scale.max] === 'undefined') {
        this._segments.colorStops[this.config.scale.max] =
          this._segments.colorStops[
      ...
      this._segments.gap =
        this.config.segments.colorstops.gap !== 'undefined'
          ? this.config.segments.colorstops.gap
          : 1;
      ...
      for (var i = 0; i < this._segments.count; i++) {
        ...
        if (this.dev.debug)
          console.log(
            'colorstuff - COLORSTOPS++ segments',
            segmentRunningSize,
            this._segmentAngles[i]
          );
      }
    ...
    this.skipOriginal =
      this.config.show.style === 'colorstops' || this.config.show.style === 'colorlist';
    ...
    if (this.skipOriginal) {
      ...
      for (var i = 0; i < this._arc.parts; i++) {
        this._segmentAngles[i] = {
          boundsStart:
            this.config.position.start_angle + i * this.config.segments.dash * this._arc.direction,
          boundsEnd:
            this.config.position.start_angle +
            (i + 1) * this.config.segments.dash * this._arc.direction,
          drawStart:
            this.config.position.start_angle +
    ...
    if (this.dev.debug)
      console.log('SegmentedArcTool - init', this.toolId, this.config.isScale, this._segmentAngles);
  ...
  _renderSegments() {
    ...
    if (this.skipOriginal) {
      ...
      if (this.dev.debug)
        console.log(
          'RENDERNEW - IN _arcId, firstUpdatedCalled',
          this._arcId,
          this._firstUpdatedCalled
        );
      ...
      arcEndPrev =
        valPrev * this._arc.size * this._arc.direction + this.config.position.start_angle;
      ...
      if (this._firstUpdatedCalled) {
        ...
        function animateSegmentsNEW(timestamp, thisTool) {
          ...
          frameSegment = thisTool._segmentAngles.findIndex((currentValue, index) =>
            thisTool._arc.clockwise
              ? tween.frameAngle <= currentValue.boundsEnd &&
                tween.frameAngle >= currentValue.boundsStart
              : tween.frameAngle <= currentValue.boundsStart &&
                tween.frameAngle >= currentValue.boundsEnd
          );

          if (frameSegment === -1) {
            /* if (thisTool.debug) */ console.log(
              'RENDERNEW animateSegments frameAngle not found',
              tween,
          ...
          runningSegment = thisTool._segmentAngles.findIndex((currentValue, index) =>
            thisTool._arc.clockwise
              ? tween.runningAngle <= currentValue.boundsEnd &&
                tween.runningAngle >= currentValue.boundsStart
              : tween.runningAngle <= currentValue.boundsStart &&
                tween.runningAngle >= currentValue.boundsEnd
          );
          ...
          do {
            ...
            if (as) {
              ...
              if (thisTool.config.show.style === 'colorlist') {
                ...
                thisTool.styles.foreground[runningSegment].fill =
                  thisTool.config.segments.colorlist.colors[runningSegment];
              }
              // #WIP
              // Testing 'lastcolor'
              if (thisTool.config.show.lastcolor) {
                ...
                const value = Math.min(
                  Math.max(0, (runningSegmentAngle - boundsStart) / (boundsEnd - boundsStart)),
                  1
                );
            ...
            runningSegment = thisTool._segmentAngles.findIndex((currentValue, index) =>
              thisTool._arc.clockwise
                ? runningSegmentAngle <= currentValue.boundsEnd &&
                  runningSegmentAngle >= currentValue.boundsStart
                : runningSegmentAngle <= currentValue.boundsStart &&
                  runningSegmentAngle >= currentValue.boundsEnd
            );
            ...
            tween.runningAngle !== tween.frameAngle /* && (runningSegment == runningSegmentPrev) */
          );
        ...
        if (
          /* (val != valPrev) && */ this._card.connected === true &&
  ...
  buildArcPath(argStartAngle, argEndAngle, argClockwise, argRadiusX, argRadiusY, argWidth) {
    ...
    const start2 = this.polarToCartesian(
      this.svg.cx,
      this.svg.cy,
      cutoutRadiusX,
      cutoutRadiusY,
    ...
    const end2 = this.polarToCartesian(
      this.svg.cx,
      this.svg.cy,
      cutoutRadiusX,
      cutoutRadiusY,
